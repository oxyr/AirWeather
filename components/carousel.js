...
const ELEVATION = 0.1;//Math.cos(Math.PI / 2.5);
const ROTATION_RATE = Platform.OS === "ios" ? 5 : 5;
const PAN_ROTATION_RATE = 0.1;
...
export default class CircularCarousel extends Component {
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (
            ...
            console.log("middleItemAngle",middleItemAngle)
	...
	renderItem(data, index) {
		...
		const _itemStyle = {
			...Styles.itemStyle,
			marginTop: item.frame.x,
			marginLeft: index==0?50+0:50+2.5    ,//item.frame.x,
			zIndex: item.zIndex,
			width: item.frame.w,
			height: item.frame.h,
			backgroundColor: data.color,
			opacity: 1,//item.opacity
		};
	...
	static rearrangeItems(toAngle, state, props) {
		...
		let marginY = itemDim.height;

		items.forEach((item) => {
            let _angle = (item.angle + toAngle + 360) % 360;
			let alpha = _angle * (Math.PI / 180); // Radian conversion
            let x = radius * Math.sin(alpha) + marginX;
			let y = radius * Math.cos(alpha) * ELEVATION*1 + marginY;
            console.log("item angle:",_angle,"x:",x,"y:",y,marginX)
			item.angle = _angle;
